...
$allSendAsPermissions = foreach ($mailbox in $userMailboxes) {
...
$allSendAsPermissions | Export-Csv -Path "$outputFolder\UserMailboxSendAsPermissions.csv" -NoTypeInformation -Encoding UTF8

# Notify that the export was successful
Write-Host "SendAs permissions exported to $outputFolder\UserMailboxSendAsPermissions.csv"
